...
def train_model_plotly(
    ...
    focal_alpha=None,  # Class balancing factor (default: None)
    dynamic_gamma=False,  # Enable gamma adjustment
    gamma_schedule=None  # Function for updating gamma
):
    """
    Trains a neural network model with options to change learning rates during training.

    Parameters:
    -----------
    csv_path : str
        Path to the CSV file containing metadata (default: "metadata_modelnet10.csv").
    batch_size : int
        Batch size for training and testing (default: 16).
    remove_indices : list
        List of indices to remove from the dataset (default: [4455, 4456, 4557]).
    hidden_dim : int
        Hidden dimension for the neural network (default: 64).
    lr : float
        Initial learning rate for the optimizer (default: 0.01).
    weight_decay : float
        Weight decay (L2 regularization) for the optimizer (default: 1e-5).
    num_epochs : int
        Number of training epochs (default: 20).
    save_path : str, optional
        Path to save the final model; if None, a default name is used (default: None).
    confusion_interval : int
        Interval (in epochs) to print the confusion matrix for the test set (default: 5).
    save_best : bool
        Whether to save the best model based on test accuracy (default: True).
    early_stopping : bool
        Whether to use early stopping based on test loss (default: False).
    patience : int
        Epochs to wait for improvement before early stopping (default: 10).
    scheduler_type : str, optional
        Type of learning rate scheduler to use. Options: None, 'plateau', 'step', 'exponential'.
        Default: None (no scheduler).
    scheduler_params : dict, optional
        Parameters for the chosen scheduler. Examples:
        - For 'plateau': {'factor': 0.5, 'patience': 5}
        - For 'step': {'step_size': 10, 'gamma': 0.1}
        - For 'exponential': {'gamma': 0.95}
        Default: {} (uses scheduler defaults if not specified).
    log_file : str, optional
        Path to log metrics to a CSV file; if None, no logging (default: None).
    """
    # Fixed parameters (modify as needed)
    num_classes = 10
    num_layers = 2
    root_path = 'Data/' + root_path # Change to your data path
    # Load and split data (assuming split_train_test and MeshDataset are defined)
    df = pd.read_csv(csv_path)
    df['path'] = root_path + df['object_path']
    train_df, test_df = split_train_test(df, remove_indices)
    train_dataset = MeshDataset(train_df)
    test_dataset = MeshDataset(test_df)
    ...
    if save_path is None:
        base_name = f"Saved\\SNN_Focal_{hidden_dim}_{batch_size}_{num_layers}_{num_epochs}"
        final_save_path = base_name + "_final.pt"
        best_save_path = base_name + "_best.pt" if save_best else None
    else:
        final_save_path = save_path
        best_save_path = save_path.replace(".pt", "_best.pt") if save_best else None

    # Initialize model, optimizer, and loss (assuming SNN is your model class)
    model = SNN(hidden_dim=hidden_dim, num_classes=num_classes, num_layers=num_layers).to(device)
    ...
    # Tracking variables
    best_test_acc = 0
    best_test_loss = float('inf') if early_stopping else None
    ...
    for epoch in range(num_epochs):
        ...
        if dynamic_gamma and gamma_schedule:
            focal_gamma = gamma_schedule(epoch)  # Update gamma based on the schedule
            criterion.gamma = focal_gamma  # Apply new gamma to Focal Loss
        
        # Print status
        print(f"Epoch {epoch+1}/{num_epochs} - Train Loss: {avg_train_loss:.4f}, Train Acc: {train_acc:.4f}, Gamma: {focal_gamma:.2f}")
        ...
        if (epoch + 1) % confusion_interval == 0:
            ...
            fig_train = px.imshow(cm_train_df, text_auto=True, color_continuous_scale='Reds',
                                  labels=dict(x="Predicted Label", y="True Label", color="Count"))
            fig_train.update_layout(title=f"Training Confusion Matrix - Epoch {epoch + 1}",
            ...
            cm_test_df = pd.DataFrame(cm_test, 
                                      index=[f'Class {i}' for i in range(num_classes)], 
                                      columns=[f'Class {i}' for i in range(num_classes)])
            fig_test = px.imshow(cm_test_df, text_auto=True, color_continuous_scale='Blues',
                                 labels=dict(x="Predicted Label", y="True Label", color="Count"))
            fig_test.update_layout(title=f"Test Confusion Matrix - Epoch {epoch + 1}",
                                   xaxis_title="Predicted Label", 
                                   yaxis_title="True Label")
            fig_test.show()
        ...
        if save_best and test_acc > best_test_acc:
            ...
            print(f"New best model saved to {best_save_path} with test acc: {test_acc:.4f}")
        ...
        if log_file:
            with open(log_file, 'a') as f:
                f.write(f"{epoch + 1},{avg_train_loss:.4f},{train_acc:.4f},{avg_test_loss:.4f},{test_acc:.4f}\n")

    # Save final model
    torch.save(model.state_dict(), final_save_path)
    print(f"Final model saved to {final_save_path}")

def train_saved_model_plotly(
    saved_model_path = "SNN_Focal_64_32_2_20_final.pt",
    csv_path="metadata_modelnet10.csv",
    root_path="half_edge_structures",
    batch_size=32,
    remove_indices=[4455, 4456, 4557],
    ...
    # Define save paths
    if save_path is None:
        base_name = f"Saved\\SNN_Extra_Focal_{hidden_dim}_{batch_size}_{num_layers}_{num_epochs}"
        final_save_path = base_name + "_final.pt"
        best_save_path = base_name + "_best.pt" if save_best else None
    else:
        final_save_path = save_path
    ...
    # Initialize model, optimizer, and loss (assuming SNN is your model class)
    model = SNN(hidden_dim=hidden_dim, num_classes=num_classes, num_layers=num_layers).to(device)
    saved_model_path = "Saved\\" + saved_model_path
    # Load saved model
...
def train_enhanced_model(
    ...
    num_classes = 10
    features = [7, 2, 5]  # Feature dimensions for vertices, edges, and faces
    ...
    train_dataset = MeshDataset(data_list_train)
    test_dataset = MeshDataset(data_list_test)
    ...
    # Define save paths
    if save_path is None:
        base_name = f"Saved/Enhanced_SNN_hd{hidden_dim}_lr{lr}_wd{weight_decay}_epochs{num_epochs}_seed{seed}_bs{batch_size}_drop{dropout_rate}_bn{int(use_batch_norm)}"
        final_save_path = base_name + "_final.pt"
...
def train_multiple_datasets(
    ...
    # Define save paths
    if save_path is None:
        base_name = f"Saved/Augmented_SNN_hd{hidden_dim}_lr{lr}_wd{weight_decay}_epochs{num_epochs}_seed{seed}_bs{batch_size}_drop{dropout_rate}_bn{int(use_batch_norm)}"
        final_save_path = base_name + "_final.pt"
        best_save_path = base_name + "_best.pt" 
    else:
        final_save_path = save_path
